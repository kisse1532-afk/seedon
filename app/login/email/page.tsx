...
export default function IdLoginPage() {
  ...
  function toKorean(raw: string): string {
    ...
    if (m.includes("email not confirmed"))
      ...
      //
      // 2026.08.25 확인: 지금은 이 상황이 안 생긴다. 가입하면 그 자리에서
      // 바로 로그인되게 설정돼 있다(계정을 하나 만들어 확인했다 —
      // 확인 메일이 나간 적이 없고 가입 즉시 확인된 상태로 만들어졌다).
      // 그래도 이 안내는 남겨둔다. 설정이 언젠가 바뀌면 청소년이 영문 오류를
      // 그대로 보게 되기 때문이다. 그물은 안 걷는다.
