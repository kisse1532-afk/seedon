 ...
 * - 개인정보 처리방침(lib/policy.ts)에 Anthropic을 2026.08.26에 미리 넣어뒀다 —
 *   "켜져 있을 때는"이라는 조건문이라 키가 꺼진 지금도 거짓이 아니다.
 *   키를 켤 때 방침을 또 고칠 필요 없다.
...
- reason은 청소년에게 그대로 보여준다. 한 문장, 35자 안쪽, ~해요체(반말 금지)
- **청소년의 형편·처지·자격을 말하지 마라. 받을 수 있는지 없는지도 말하지 마라** —
  씨드온은 자격을 판정하지 않는다. 카드가 무엇을 해주는지와, 적어준 말 중
  무엇을 보고 골랐는지만 쓴다
  (금지: "~한 분이시군요" "조건에 해당돼요" "~라면 받을 수 있어요")
  (좋은 예: "학원비가 부담된다고 적어줘서 이걸 골랐어요")
- 쓰면 안 되는 말: 저소득, 지원대상자, 취약계층, 결손가정, 차상위계층, 수급자,
  중위소득, 소득인정액, 연계, 이수, 산정, 선정
- 중학생이 사전 없이 읽을 수 있는 말만 써라
- 자해·자살·심한 폭력·방금 집을 나온 상황 같은 위기 신호가 보이면 crisis를 true로 하라.
  그래도 picks는 채워라(심리상담·주거 카드 등)`;

/* 지시문은 확률이라 뚫린다. 화면에 그대로 나가는 문장이므로 기계로 한 번 더 거른다.
   걸리면 그 카드를 버리는 게 아니라 이유 문장만 버린다 — 화면은 reason이 없어도 그린다. */
const BANNED = /저소득|지원\s*대상자|취약\s*계층|결손\s*가정|차상위|수급자|중위소득|소득인정액|분이시|해당[돼되]|받을 수 있어요/;
...
export async function aiRecommend(q: string, programs: Program[]): Promise<AiResult | null> {
  ...
  try {
    ...
    const picks = parsed.picks
      ...
      // 낙인·판정 문장은 이유만 지운다. 카드는 그대로 보여준다
      .map((p) => (BANNED.test(p.reason) ? { ...p, reason: "" } : p));
